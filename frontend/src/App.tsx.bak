          ...
          {activeTab === 'canvas' && (
            <div className="space-y-6 w-full">
              ...
              <div className="glass-panel p-6">
                ...
                <div style={{ marginBottom: '20px', padding: '14px', background: 'rg
                  ...
                  <div className="space-y-4 bg-slate-900/20 p-4 rounded-xl border border-slate-850/60">
                    ...
                    {/* Saved logo picker with thumbnails */}
                    {savedLogos.length > 0 && (
...
print("UI Redesign modifications finished!")
                            ...
                            {canvasSelectedItem && canvasSelectedItem.media_paths && canvasSelectedItem.media_paths.length > 1 && (
                              <>
                                ...
                                >
                                  ...
                                  className="absolute left-2.5 top-1/2 -translate-y-1/2 z-30 w-8 h-8 rounded-full bg-slate-950/80 border border-slate-850 text-slate-300 hover:text-white flex items-center justify-center transition-all hover:bg-slate-900 active:scale-95 hover:scale-105 shadow-md shadow-black/40 font-black text-xs select-none pointer-events-auto"
                                  title="ดูรูปพื้นหลังก่อนหน้า"
                                >
                                  ◀
                                </button>
                                <button
                                  ...
                                  onClick={(e) => {
                                    ...
                                    if (failedCount > 0) {
                                      const firstError = data.results.find((r: any) => r.error)?.error || 'Unknown error';
                                        ...
                                        alert(`การอัพโหลดบางส่วนล้มเหลว (${failedCount}/${data.results.length} 
                                      }
                                    }
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        ...
                                        setDropboxUploadProgress('กำลังเริ่มอัพโหลด...');
                                          ...
                                          const filePaths = contentGraphics.map((g: any) => g.file_path);
                                      disabled={isUploadingDropbox || !dropboxToken}
                                      onClick={async () => {
                                        if (!dropboxToken) {
                                          alert('กรุณากรอก Dropbox Access Token ในส่วนตั้งค่าพื้นฐานก่อน');
                                          return;
      ...
      for (const result of successfulResults) {
        ...
        if (matchingGraphic) {
          ...
          if (item) {
            ...
            headline = copywriting?.headline_3line?.join(' ') || item.selected_headline || item.title || '';
                                  ...
                                  <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opac
                                    <Eye className="w-5 h-5 text-emerald-400 filter drop-shadow" />
                                      e.stopPropagation();
                                      setGraphicIndex(prev => (prev === contentGraphics.length - 1 ? 0 : prev + 1));
                                    }}
                                    className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-slate-950/80 border border-slate-800 hover:border-emerald-400 text-slate-400 hover:text-white flex items-center justify-center transition-all cursor-pointer shadow-lg hover:scale-105 active:scale-95 z-10"
        ...
        addLog(` - [แมปข้อมูลสำเร็จ] ไฟล์: ${result.file_path.split('/').pop()} -> หัวข
                                         csvRows.push([
                                           escapeCsv(headline),
                                           escapeCsv(caption),
                                           escapeCsv(result.shared_link || ''),
