 # Walkthrough - Graphic Canvas Backdrop Selection Persistence & Styling Adjustments
 
 I have successfully resolved the backdrop frame selection persistence issue and applied visual improvements to the Pillow image generator script.
 
 ## Changes Made
 
 ### 1. Persistent Selected Background Image Logic
 - **File Modified**: [App.tsx](file:///Users/paulpolsulintaboon/Documents/GitHub/ContentFactory/frontend/src/App.tsx)
 - **New Helper Function**:
   - `handleSelectCanvasBgImage(item, path)`: Sets the current background preview state (`canvasBgImage`), updates the item's metadata in memory (`selected_bg_image` under `metadata.copywriting`), and silently performs a `PUT` request to `/api/vault/contents/${item.id}/metadata` to persist the chosen background image to the SQLite database instantly in the background.
 - **Loading Saved Selection**:
   - Updated the detail loader `useEffect` hook to read `selected_bg_image` from the article's copywriting metadata. If present, it restores the user's selected frame; otherwise, it falls back to the first available media path.
 - **UI Binding & Actions**:
   - Bound the new helper function to the frame thumbnail list item click action.
   - Bound the helper to the cycler back (`◀`) and forward (`▶`) arrow buttons overlaying the preview.
   - Updated the manual copywriting save button (`💾 บันทึกการแก้ไข`) to preserve the selected image path in its SQLite update payload.
 
 ### 2. Backdrop Image Scaling & Blurred Ba
     3. Performs a silent background `PUT` request to `/api/vault/contents/${item.id}/metadata` to update the item's metadata in the SQLite database instantly.
 
 - **Update UI Actions**:
   - Scales the 16:9 foreground image to fit the width of the vertical canvas without cropping, pasting it centered on top of a heavily blurred and darkened copy of itself.
 - **Prevent Excessive Zooming (top_gainers Split)**:
   - In `top_gainers` layout, updated the `prepare_base_canvas` invocation to pass the split height `image_split_y` instead of the full canvas height. This ensures the background image is scaled to fit the actual top portion height, minimizing scaling zoom from 2.66x to a natural 1.33x.
 
 ### 3. Top-Left & Bottom-Right Channel Badge Proportions
 - **File Modified**: [06_graphic_generator.py](file:///Users/paulpolsulintaboon/Documents/GitHub/ContentFactory/scripts/06_graphic_generator.py)
 - **Shorthand Subscriber Count Label**:
   - Updated the subscriber formatting functions to use `"Sub"` instead of the full `"subscriber"` / `"subscribers"` string (e.g. `42.7K Sub` instead of `42.7k subscribers`).
     - Line 1: Cyber YouTube Red box `(255, 0, 51)` with White text.
     - Line 2: Cyber Neon Yellow box `(255, 230, 0)` with Dark Charcoal `(10, 10, 10)` text.
     - Line 3: Pure White box with Cyber YouTube Red text.
 - **Dynamic Sticker Tilt**:
   - Replaced flat rectangles by applying alternating rotation angles in `draw_tilted_word_layer` calls in the `top_gainers` layout:
     - Even lines tilt left at `-0.03` radians (approx -1.7°).
     - Odd lines tilt right at `0.02` radians (approx 1.1°).
     - This provides a natural, high-engagement sticker placement look.
 
 ### 3. Fixed Dropbox + CSV Export Duplication
 - **File Modified**: [App.tsx](file:///Users/paulpolsulintaboon/Documents/GitHub/ContentFactory/frontend/src/App.tsx)
 - **Problem**:
